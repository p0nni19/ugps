...
public class PackageService {
	...
	public List<PackageResponse> findNearbyPackages(LocationRequest location) {
		...
		return allPackages.stream().map(pkg -> {
			double distance = calculateDistance(location.getLat(), location.getLongi(), pkg.getFromLat(),
					pkg.getFromLong());

			if (distance <= 5) {
				double cost = calculateDistanceCost(distance);
				return new PackageResponse(pkg.getPackageId(), pkg.getFromLat(), pkg.getFromLong(), pkg.getToLat(),
						pkg.getToLong(), pkg.getFromAddress(), pkg.getToAddress(), distance, cost, pkg.getStatus());
			} else {
				return null;
			}
